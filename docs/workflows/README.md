...
5. **실행 계획:** 브랜치가 `exec-plan/<slug>`이고 `docs/exec-plans/active/exec-plan-<slug>.md`가 있으면, PR 생성 직후 **`docs/exec-plans/completed/`** 로 옮기고 하단에 PR·시각을 덧붙입니다. 생략: `EXEC_PLAN_NO_MOVE=1`. 자세한 설명은 [`../exec-plans/README.md`](../exec-plans/README.md).
