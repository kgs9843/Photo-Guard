...
- `npm run dev:5231` — 로컬 확인용 Vite (`127.0.0.1:5231`). **Playwright `test:e2e`는 기본 포트 `5237`** 을 쓰며 `dev:5231`과 겹치지 않음.
- `npm run test:e2e` — Playwright (`playwright.config.ts` + `e2e/`). 최초 `npx playwright install chromium`. 선택 실행: `npx playwright test e2e/settings-history-toast.spec.ts`
...
| `PLAYWRIGHT_E2E_PORT`       | `5237`                            | `playwright.config.ts`의 Vite·`baseURL` 포트 (`test:e2e`)                       |
